...
import warnings
warnings.warn("NicksIntervals.Interval and NicksIntervals.MultiInterval are deprecated. Please use Interval and Multi_iInterval instead.", DeprecationWarning)
...
class Old_Interval:
	...
	@classmethod
	def make_infinite_full(cls) -> Old_Interval:
		"""
		...
		return Old_Interval(-float('inf'), float('inf'))
	
	@classmethod
	def make_infinite_empty(cls) -> Old_Interval:
		"""
	...
	def get_ordered(self) -> Old_Interval:
		return Old_Interval(
			min(self.start, self.end),
			max(self.start, self.end)
		)
	
	def intersect(self, other: Union[Old_Interval, Old_Multi_Interval]) -> Union[Old_Interval, Old_Multi_Interval, None]:
		"""
		...
		:return:
		"""
		if isinstance(other, Old_Multi_Interval):
			result = Old_Multi_Interval([])
			...
			if not result.intervals:
				...
				return result
		elif isinstance(other, Old_Interval):
	...
	def intersects(self, other: Old_Interval) -> bool:
		if isinstance(other, Old_Multi_Interval):
			result = Old_Multi_Interval([])
			for sub_interval in other.intervals:
	...
	def contains(self, other: Old_Interval, allow_inside_touching: bool = False) -> bool:
		if isinstance(other, Old_Multi_Interval):
	...
	def relate_DE_9IM(self, other: Old_Interval):
	...
	def union(self, other: Old_Interval) -> Union[Old_Interval, Old_Multi_Interval]:
		if self.intersect(other) is not None:
			return self.hull(other)
		else:
			return Old_Multi_Interval((self, other))
	
	def hull(self, other: Old_Interval) -> Old_Interval:
		return type(self)(min(self.start, other.start), max(self.end, other.end))
	
	def touches(self, other: Old_Interval):
		"""
	...
	def subtract(self, other: Union[Old_Interval, Old_Multi_Interval]) -> Union[Old_Interval, Old_Multi_Interval, None]:
		if isinstance(other, Old_Multi_Interval):
			result = self.copy()
			for sub_interval in other.intervals:
			...
			if self.start <= other.start:
				...
				#  |-------|
				#    |---|
				return Old_Multi_Interval((type(self)(self.start, other.start), type(self)(other.end, self.end)))
...
class Old_Multi_Interval:
	
	def __init__(self, iter_intervals: Iterable[Old_Interval]):
		""" Intervals provided to this initialiser are added without further processing.
		"""
		self.intervals: List[Old_Interval] = []
	...
	def copy(self) -> Old_Multi_Interval:
	...
	def subtract(self, interval_to_subtract: Old_Interval):
		new_interval_list = []
	...
	def add_overlapping(self, interval: Union[Old_Interval, Old_Multi_Interval]) -> Old_Multi_Interval:
		...
		:return: self
		"""
		if isinstance(interval, Old_Interval):
			self.intervals.append(interval.copy())
		elif isinstance(interval, Old_Multi_Interval):
	...
	def add_hard(self, interval_to_add: Old_Interval) -> Old_Multi_Interval:
	...
	def add_soft(self, interval_to_add: Old_Interval) -> Old_Multi_Interval:
		...
		if isinstance(interval_to_add, Old_Multi_Interval):
			for sub_interval in interval_to_add.intervals:
				self.intervals.append(sub_interval.copy())
		elif isinstance(interval_to_add, Old_Interval):
			self.add_overlapping(interval_to_add)
	...
	def add_merge(self, interval_to_add: Old_Interval) -> Old_Multi_Interval:
	...
	def intersection(self, arg_interval: Union[Old_Interval, Old_Multi_Interval]) -> Old_Multi_Interval:
		if isinstance(arg_interval, Old_Multi_Interval):
			result_multi_interval: Old_Multi_Interval = type(self)([])
		...
		elif isinstance(arg_interval, Old_Interval):
			result_multi_interval: Old_Multi_Interval = type(self)([])
			for my_interval in self.intervals:
	...
	def merge_touching_and_intersecting(self) -> Old_Multi_Interval:
		"""
	...
	def delete_infinitesimal(self) -> Old_Multi_Interval:
	...
	def hull(self) -> Union[Old_Interval, None]:
